...
try {
    ...
    String query = "SELECT email FROM users"; // Query to select all emails from the users table
    ps = conn.prepareStatement(query);
    rs = ps.executeQuery();
%>
<label for="months" style="margin-left:30px;">Email:</label>
<select id="months" name="email" required>
    <%
    while (rs.next()) {
...
//        ResultSet rs = null;
        ...
        try {
            ...
            if (request.getParameter("showdata") != null) {
             ...
             selectedEmail = request.getParameter("email"); // Get selected email from the request

            // Construct the SQL query with additional conditions for selected year and email
            String query = "SELECT s.*, u.names, u.email FROM savingtb s INNER JOIN users u ON s.userid = u.userid WHERE YEAR(s.dates) = ? AND u.email = ? ORDER BY s.savingid DESC";
