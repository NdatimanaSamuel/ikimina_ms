...
try {
    calculationText = ""; // Reset calculationText at the beginning of the try block
    ...
    if (request.getParameter("showdata") != null) {
        ...
        String creditQuery = "SELECT SUM(requestedamount) AS total_requested FROM creditb WHERE YEAR(dates) = ? AND paystatus = 'not paid' AND status = 'approved'";
        ...
        String query = "SELECT s.*, u.names, u.email FROM savingtb s INNER JOIN users u ON s.userid = u.userid WHERE YEAR(s.dates) = ? ORDER BY s.savingid DESC";
...
//                totalAmount += totalCreditRequested;
                ...
                // Calculate and display the result of total saving minus total credit
                int finalResult = totalAmount - totalCreditRequested;
                 calculationText = totalAmount + " - " + totalCreditRequested + " = " + finalResult;
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
...
<div style="text-align: right; margin-top: 10px;">
    <strong>Calculation: <%= calculationText %> Frw</strong>
</div>
