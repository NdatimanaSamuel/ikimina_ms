  ...
  <header>
   ...
   <h1>Ikimina Ms SAVE & CREDIT( Welcome, <%= session.getAttribute("Name") %> )</h1>
            ...
            <div class="add-book-form">
                <h2>Make Payment of Credit</h2>
                 <!-- Retrieve user ID from the URL parameter -->
                     <%
        String userIdStr = request.getParameter("creditId");
        if (userIdStr != null) {
            int creditId = Integer.parseInt(userIdStr);
            String ReAmount = ""; // Initialize the username
            ...
            try {
                ...
                String query = "SELECT requestedamount FROM creditb WHERE creditid=?";
                ...
                if (resultSet.next()) {
                    ReAmount = resultSet.getString("requestedamount");
    ...
    %>
                <form action="GuestPayMoney" method="post">
              <input type="hidden" id="title" name="creditid" value="<%= creditId %>">
              <!-- New fields for amount and month -->
              <label for="amount">Requested Amount:</label>
              <input type="text" id="amount" name="amount"  value="<%= ReAmount %>">
              <!-- Calculate amount to pay (requested amount + 5%) -->
                <% 
                double requestedAmount = Double.parseDouble(ReAmount);
                double amountToPay = requestedAmount + (requestedAmount * 0.05); // Adding 5% to the requested amount
                %>
                  <label for="amountToPay">Amount to Pay With 5% Increase:</label>
                <input type="text" id="amountToPay" name="amountToPay" value="<%= amountToPay %>" readonly>
             ...
             <select id="status" name="status">
              ...
              <!-- Submit button -->
              <button type="submit" value="Save Changes"  style="width:250px;">Pay</button>
          </form>
 <% } %>
            </div>
